...
export function BidModal({ slot, onClose }: Props) {
  ...
  const insufficientBalance = amount > 0 && (state.user?.wallet ?? 0) < amount;
  const belowMinBid = amount > 0 && amount < minBid;
  ...
  async function handleImageSelect(file: File) {
    ...
    if (!['image/jpeg', 'image/png', 'image/webp', 'image/gif'].includes(file.type)) { setUploadError('Use JPEG, PNG, WebP, or GIF'); return; }
    setImageFile(file);
    ...
    try {
      ...
      if (!res.ok) { setUploadError(data.error ?? 'Upload failed'); setImageFile(null); setImagePreview(null); }
      else { setImagePath(data.path); }
    } catch {
  ...
  function clearImage() {
    setImageFile(null); setImagePreview(null); setImagePath(null); setUploadError(null);
  }
  ...
  async function handleBid() {
    ...
    try {
      const res = await fetch('/api/bid', {
        ...
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          userId: state.user.id, slotId: slot.id, bidAmount: amount,
          durationHours: duration, adText: adText.trim(), adUrl: adUrl.trim() || null,
          adEmoji, adColor, adImagePath: imagePath,
        }),
      });
      const data = await res.json();
      if (!res.ok || data.error) {
        ...
        showToast(occ ? `You seized "${slot.name}"! Previous holder gets their refund.` : `You claimed "${slot.name}"! Your ad is live.`, 'success');
        if (data.user) {
        ...
        <div className="px-4 pb-6">
          <div className="flex items-start justify-between mb-4">
            <div>
              ...
              <span className={clsx('text-xs font-bold px-2 py-0.5 rounded', `tier-${slot.tier}`)}>{slot.tier.toUpperCase()}</span>
            </div>
            ...
            <div className="rounded-xl border border-brand-border bg-brand-surface p-3 mb-4">
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-brand-muted font-mono">{formatGramsShort(occ.bid_amount)} GRAM bid · {timeStr} left</p>
                </div>
                {occ.ad_image_path && (
                  // eslint-disable-next-line @next/next/no-img-element
                  <img src={adImageUrl(occ.ad_image_path) ?? ''} alt="" className="w-10 h-10 rounded-lg object-cover flex-shrink-0" />
                )}
              </div>
              {!isOwner && (
              ...
              {!isOwner && (
                <button onClick={() => setShowReport(true)} className="mt-2 text-xs text-red-400/80 hover:text-red-300 underline">
          ...
          <div className="mb-4">
            <label className="text-xs font-bold text-brand-muted uppercase tracking-wider block mb-1.5">Your Bid (GRAM)</label>
            <div className="relative">
              <input
                type="number" value={bidAmount} onChange={e => setBidAmount(e.target.value)}
                step="0.0001" min={minBid}
                className={clsx('w-full bg-brand-surface border rounded-xl px-4 py-3 text-lg font-bold font-mono text-brand-text outline-none',
            ...
            {belowMinBid && (
              <p className="text-xs text-red-400 mt-1.5">
                ⚠️ Minimum bid is {minBid.toFixed(4)} GRAM{occ ? ` (${slot.min_increment_pct}% above current bid)` : ''}
              </p>
            )}
            {insufficientBalance && !belowMinBid && (
              <p className="text-xs text-red-400 mt-1.5">
                ⚠️ Insufficient balance — you have {formatGramsShort(state.user?.wallet ?? 0)} GRAM
              </p>
            )}
              ...
              {DURATIONS.map(h => (
                <button key={h} onClick={() => setDuration(h)}
                  className={clsx('py-2 rounded-lg text-xs font-bold transition-all',
                    duration === h ? 'bg-brand-gold text-brand-dark' : 'bg-brand-surface border border-brand-border text-brand-muted')}>
                  {h}h
                </button>
              ))}
            </div>
          ...
          <div className="mb-4">
            <label className="text-xs font-bold text-brand-muted uppercase tracking-wider block mb-1.5">Ad Message <span className="text-red-400">*</span></label>
            <input type="text" value={adText} onChange={e => setAdText(e.target.value.slice(0, 60))} maxLength={60}
          ...
          <div className="mb-4">
            <label className="text-xs font-bold text-brand-muted uppercase tracking-wider block mb-1.5">Link (optional)</label>
            <input type="url" value={adUrl} onChange={e => setAdUrl(e.target.value)} placeholder="https://t.me/yourchannel"
              className="w-full bg-brand-surface border border-brand-border focus:border-brand-gold/40 rounded-xl px-4 py-3 text-sm text-brand-text outline-none" />
          </div>

          <div className="mb-4">
            <label className="text-xs font-bold text-brand-muted uppercase tracking-wider block mb-1.5">Image (optional)</label>
            {imagePreview ? (
              <div className="relative rounded-xl overflow-hidden border border-brand-border">
                ...
                <button onClick={clearImage} className="absolute top-2 right-2 w-7 h-7 rounded-full bg-black/70 text-white text-sm flex items-center justify-center">×</button>
                {imagePath && <span className="absolute bottom-2 left-2 text-[10px] bg-green-900/80 text-green-300 px-2 py-0.5 rounded-full">✓ Uploaded</span>}
              </div>
            ) : (
            ...
            <p className="text-[10px] text-brand-muted mt-1.5">Images are public and subject to the content policy below.</p>
          </div>

          <div className="mb-4">
            <label className="text-xs font-bold text-brand-muted uppercase tracking-wider block mb-1.5">Icon</label>
            <div className="flex flex-wrap gap-2">
              {EMOJIS.map(e => (
                <button key={e} onClick={() => setAdEmoji(e)}
                  className={clsx('w-9 h-9 rounded-lg text-xl transition-all', adEmoji === e ? 'bg-brand-gold/20 ring-1 ring-brand-gold' : 'bg-brand-surface')}>
                  {e}
                </button>
              ))}
            </div>
          ...
          <div className="mb-6">
            <label className="text-xs font-bold text-brand-muted uppercase tracking-wider block mb-1.5">Brand Color</label>
            <div className="flex gap-2 flex-wrap">
              {COLORS.map(c => (
                <button key={c} onClick={() => setAdColor(c)}
                  className={clsx('w-8 h-8 rounded-full transition-all border-2', adColor === c ? 'border-white scale-110' : 'border-transparent')}
                  style={{ background: c }} />
              ))}
            </div>
          </div>
          ...
          <div className="mb-3 rounded-xl border border-red-900/40 bg-red-950/20 p-3">
            ...
            <p className="text-[11px] text-red-200/80 leading-relaxed">
              ...
              and your stake for that slot is forfeited — not refunded. By bidding, you confirm the content is yours to use and lawful to display.
            </p>
          </div>
          ...
          <label className="flex items-start gap-2.5 mb-4 cursor-pointer">
            ...
            <span className="text-xs text-brand-muted leading-relaxed">
              I confirm this content does not violate anyone's rights and is lawful. I understand it may be removed immediately and my stake forfeited if it isn't.
            </span>
          </label>
          ...
          <button
            ...
            className={clsx('w-full py-4 rounded-xl font-bold text-base transition-all',
              isValid && !loading && !insufficientBalance
      ...
      {showReport && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center modal-backdrop p-4" onClick={(e) => e.target === e.currentTarget && setShowReport(false)}>
          <div className="w-full max-w-sm bg-brand-card rounded-2xl border border-brand-border p-4">
            ...
            <p className="text-xs text-brand-muted mb-3">Tell us why this content may infringe rights or break the law.</p>
            <textarea value={reportReason} onChange={e => setReportReason(e.target.value.slice(0, 300))}
              placeholder="e.g. uses my copyrighted image without permission..." rows={3}
            ...
            <div className="flex gap-2 mt-3">
              <button onClick={() => setShowReport(false)} className="flex-1 py-2.5 rounded-xl text-sm font-medium bg-brand-surface text-brand-muted">Cancel</button>
